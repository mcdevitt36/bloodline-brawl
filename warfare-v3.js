...
function buildHaunted(){
  ...
  box(4,2.1,-5,.5,4.2,8,trim);box(4,2.1,7,.5,4.2,8,trim);box(12,2.1,-9,15,4.2,.45,trim);box(12,2.1,8.5,15,4.2,.45,trim);
...
function spawnPoints(mapId){return mapId==='haunted'?[new THREE.Vector3(-3,0,-10),new THREE.Vector3(14,0,-12),new THREE.Vector3(20,0,10),new THREE.Vector3(-15,0,7),new THREE.Vector3(-27,0,18),new THREE.Vector3(12,0,22),new THREE.Vector3(29,0,-19),new THREE.Vector3(-14,0,-26)]:[new THREE.Vector3(-10,0,10),new THREE.Vector3(10,0,-10),new THREE.Vector3(31,0,-5),new THREE.Vector3(-31,0,8),new THREE.Vector3(6,0,34),new THREE.Vector3(31,0,26),new THREE.Vector3(-18,0,-27),new THREE.Vector3(2,0,12)];}
function spawnWeapons(mapId){const list=mapId==='haunted'?[['pistol',-2,-5],['smg',15,-3],['rifle',-15,-8],['shotgun',28,-22],['lmg',20,11],['sniper',-29,18],['launcher',11,24],['rifle',-14,26],['shotgun',1,7],['smg',21,-11]]:[['pistol',-8,-8],['smg',10,8],['rifle',31,-4],['shotgun',-31,8],['lmg',6,35],['sniper',-31,15],['launcher',31,26],['rifle',-18,-27],['smg',4,27],['shotgun',35,-12]];list.forEach(([type,x,z])=>createPickup(type,new THREE.Vector3(x,.45,z)));}
...
function resolveWorld(actor,old){const p=actor.mesh.position;if(currentMode==='match'&&currentMap==='city'){const zones=[[0,0,16,16],[31,-5,11,11],[-31,8,11,12],[6,34,11,10],[31,26,7,7],[-18,-27,8,7],[19,-3,7,3],[-19,5,7,3],[5,21,3,8],[22,27,8,3],[-10,-20,3,9]];if(!zones.some(([x,z,hw,hd])=>Math.abs(p.x-x)<=hw&&Math.abs(p.z-z)<=hd)){p.copy(old);return;}}else{p.x=THREE.MathUtils.clamp(p.x,-55,55);p.z=THREE.MathUtils.clamp(p.z,-55,55);}for(const o of obstacles){if(Math.abs(p.x-o.mesh.position.x)<o.half.x+.46&&Math.abs(p.z-o.mesh.position.z)<o.half.z+.46&&p.y<o.mesh.position.y+o.half.y){p.x=old.x;p.z=old.z;break;}}}
