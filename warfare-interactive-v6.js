...
function glassPane(g,x,y,z,w,h,axis='z'){
  ...
  m.position.set(x,y,z);m.userData.bbBreakable=true;m.userData.hp=2;m.userData.home=m.position.clone();g.add(m);return m;
}
...
function door(g,x,y,z,w=2.2,h=3.1,axis='z',color=0x4a382f){const d=axis==='z'?box(g,x,y,z,w,h,.22,color):box(g,x,y,z,.22,h,w,color);d.userData.bbDoor=true;d.userData.open=0;d.userData.axis=axis;d.userData.baseRot=d.rotation.y;d.userData.basePos=d.position.clone();return d;}
function jumpPad(g,x,z,c=0x55c8ff,power=11){const group=new THREE.Group();group.position.set(x,.08,z);const base=new THREE.Mesh(new THREE.CylinderGeometry(1.45,1.6,.25,24),mat(0x202b36));group.add(base);const rr=ring(group,0,.18,0,1.05,c);glow(group,0,.45,0,c,2.4,7);group.userData.bbJumpPad=true;group.userData.power=power;group.userData.cooldown=new WeakMap();g.add(group);return group;}
function hazard(g,x,z,type='vent'){
  const group=new THREE.Group();group.position.set(x,0,z);group.userData.bbHazard=type;
  if(type==='vent'){const base=cyl(group,0,.35,0,1,.7,0x4c5963);for(let i=0;i<5;i++){const b=box(group,0,.78,0,.18,.05,1.25,0x88939d);b.rotation.y=i*Math.PI/5;}glow(group,0,1,0,0x8be8ff,1.4,5);}
...
function elevator(g,x,z){const e=new THREE.Group();e.position.set(x,0,z);const platform=box(e,0,.18,0,4.5,.35,4.5,0x394552);const rail1=box(e,-2.1,1.2,0,.15,2.2,4.2,0x687580);const rail2=box(e,2.1,1.2,0,.15,2.2,4.2,0x687580);const doorL=box(e,-.72,1.55,-2.2,1.4,3,.18,0x28333f);const doorR=box(e,.72,1.55,-2.2,1.4,3,.18,0x28333f);e.userData.bbElevator=true;e.userData.platform=platform;e.userData.doors=[doorL,doorR];e.userData.phase=0;e.userData.y0=0;glow(e,0,2,-2,0x67d8ff,1.5,7);g.add(e);return e;}

function setupHaunted(scene,state){const g=state.group;
  // Breakable windows on first floor / deck side.
  state.breakables=[
    glassPane(g,-9.48,2.3,-11,2.2,1.7,'x'),glassPane(g,-9.48,2.3,11,2.2,1.7,'x'),
    glassPane(g,27.48,2.35,-11,2.2,1.7,'x'),glassPane(g,27.48,2.35,11,2.2,1.7,'x'),
    glassPane(g,2.5,2.5,-16.48,2.4,1.7),glassPane(g,17.5,2.5,-16.48,2.4,1.7),
    glassPane(g,2.5,2.5,16.48,2.4,1.7),glassPane(g,18,2.5,16.48,2.4,1.7)
  ];
  state.doors=[door(g,-9.35,1.55,-1.2,2.4,3.1,'x',0x47382f),door(g,8,1.55,16.35,2.2,3.1,'z',0x4a392f)];
  // Pool float that reacts/bobs.
  ...
  // Surprise floor launch pad hidden near graveyard flank.
  state.pads=[jumpPad(g,37,14,0xb07aff,10.5)];
  // Haunted vent / ectoplasm hazard by shed; knocks actors away.
  state.hazards=[hazard(g,40,7,'shock')];
}
function setupCity(scene,state){const g=state.group;
  ...
  state.hazards=[hazard(g,30,-11,'vent'),hazard(g,7,39,'vent'),hazard(g,-27,1,'shock')];
  state.elevator=elevator(g,27,-34);
  // Shootable rooftop ad panels / glass kiosk.
  state.breakables=[glassPane(g,26,2,-27.1,4,2.5),glassPane(g,35,2,-6.1,3.2,2.2),glassPane(g,7,2,31.1,4.5,2.3)];
  state.doors=[door(g,33,1.55,-1.8,2.2,3.1,'z',0x27323d)];
}
function setupHub(scene,state){const g=state.group;state.pads=[];state.hazards=[];state.breakables=[];state.doors=[];
  // Small playful boardwalk pad so the hub itself is fun to traverse.
  state.pads.push(jumpPad(g,-8,38,0xffcf5a,9.5));
}
...
function updateDoors(state,dt){for(const d of state.doors){const target=d.userData.open?Math.PI*.48:0;d.rotation.y=THREE.MathUtils.lerp(d.rotation.y,target,.14);}}
...
function updateElevator(scene,state,time){const e=state.elevator;if(!e)return;const t=time*.00045;e.position.y=Math.max(0,Math.sin(t))*2.2;const open=Math.sin(t)>.65;const [l,r]=e.userData.doors;l.position.x=THREE.MathUtils.lerp(l.position.x,open?-1.35:-.72,.08);r.position.x=THREE.MathUtils.lerp(r.position.x,open?1.35:.72,.08);
  // Carry actors standing on the platform vertically.
  for(const a of actorNear(scene,e.getWorldPosition(new THREE.Vector3()),3.1)){if(a.mesh.position.y<e.position.y+.75)a.mesh.position.y=e.position.y+.36;}
}
...
THREE.WebGLRenderer.prototype.render=function(scene,camera){activeScene=scene;activeCamera=camera;const state=ensure(scene);const now=performance.now();const dt=.016;updateDoors(state,dt);updatePads(scene,state,now);updateHazards(scene,state,now);updateElevator(scene,state,now);updateFloat(state,now);updateShards(state,dt);return originalRender.call(this,scene,camera);};
