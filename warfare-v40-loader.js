...
const BUILD='49';
const parts=Array.from({length:33},(_,i)=>`warfare-v40-part${String(i+1).padStart(2,'0')}.txt?build=${BUILD}`);
...
try{
  if(badge)badge.textContent='V40 • LOADING CROUCH FIX';
  ...
  if(source.length<77000)throw new Error(`runtime incomplete (${source.length} chars)`);
  new Function('THREE',`${source}\n//# sourceURL=warfare-v40-runtime.js`)(THREE);
  window.__bbWarfareV40Bootstrap={build:49,core:40,parts:parts.length,sourceLength:source.length,mode:'first-person-mega-overhaul-crouchfix'};
  if(badge){badge.textContent='V40 • CROUCH FIX LIVE • BUILD 49';badge.style.background='#073b28';badge.style.borderColor='#50e69d';}
