...
const BUILD='47';
const parts=Array.from({length:28},(_,i)=>`warfare-v40-part${String(i+1).padStart(2,'0')}.txt?build=${BUILD}`);
...
try{
  if(badge)badge.textContent='V40 • LOADING MELEE HARD-FIX';
  ...
  if(source.length<68000)throw new Error(`runtime incomplete (${source.length} chars)`);
  new Function('THREE',`${source}\n//# sourceURL=warfare-v40-runtime.js`)(THREE);
  window.__bbWarfareV40Bootstrap={build:47,core:40,parts:parts.length,sourceLength:source.length,mode:'first-person-melee-hardfix'};
  if(badge){badge.textContent='V40 • MELEE SWING FIX LIVE • BUILD 47';badge.style.background='#073b28';badge.style.borderColor='#50e69d';}
