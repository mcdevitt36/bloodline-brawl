...
const BUILD='48';
const parts=Array.from({length:32},(_,i)=>`warfare-v40-part${String(i+1).padStart(2,'0')}.txt?build=${BUILD}`);
...
try{
  if(badge)badge.textContent='V40 • LOADING WARFARE MEGA OVERHAUL';
  ...
  if(source.length<76000)throw new Error(`runtime incomplete (${source.length} chars)`);
  new Function('THREE',`${source}\n//# sourceURL=warfare-v40-runtime.js`)(THREE);
  window.__bbWarfareV40Bootstrap={build:48,core:40,parts:parts.length,sourceLength:source.length,mode:'first-person-mega-overhaul'};
  if(badge){badge.textContent='V40 • WARFARE MEGA OVERHAUL LIVE • BUILD 48';badge.style.background='#073b28';badge.style.borderColor='#50e69d';}
