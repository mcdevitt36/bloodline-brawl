...
const C={
 ...
 kelly:{shirt:0xe3b84f,pants:0x3a3444,hair:0x74513b,melee:'FAST JAB'}
};
...
function recolorBase(actor,cfg){
 const p=actor.mesh.userData.parts||{};
 if(p.torso?.material)p.torso.material.color.setHex(cfg.shirt);
 p.legs?.children?.forEach(x=>x.material?.color?.setHex(cfg.pants));
}
function addGlasses(g){
 for(const x of [-.15,.15]){const t=new THREE.Mesh(new THREE.TorusGeometry(.105,.018,6,16),mat(0x171717,.45,.1));t.position.set(x,2.49,.37);g.add(t);}box(g,0,2.49,.37,.09,.025,.025,0x171717);
}
function addConnorStubble(g){
 const m=mat(0x6a4a38,.9,0);m.transparent=true;m.opacity=.62;
 const chin=new THREE.Mesh(new THREE.SphereGeometry(.30,14,9,0,Math.PI*2,Math.PI*.46,Math.PI*.42),m);chin.position.set(0,2.31,.14);chin.scale.set(.92,.72,.82);g.add(chin);
}
function addSeanGuitar(g){
 sphere(g,.43,1.45,-.28,.27,0xb87832,.72,1.15,.42);cyl(g,.43,1.82,-.29,.055,.82,0x754725);
}
function addPaintbrush(g){
 const w=new THREE.Group();w.name='BB_CLASSIC_MELEE';w.position.set(.62,1.45,.03);w.rotation.z=-.30;g.add(w);
 cyl(w,0,.20,0,.045,.70,0x7b4b2a);box(w,0,.61,0,.22,.18,.10,0xd9d0b2);box(w,0,.71,.01,.20,.08,.11,0x3ba6d8);
}
...
function addShoes(g,key){
 const white=['erin','liam'].includes(key);for(const x of [-.24,.24])box(g,x,.12,.13,.34,.17,.52,white?0xf5f5f2:0x292b2e);
}
...
function syncRoster(){
 document.querySelectorAll('.fighter-choice').forEach(b=>{
   ...
   b.dataset.classicMelee=cfg.melee;b.title=`Classic melee: ${cfg.melee}`;
 });
}
